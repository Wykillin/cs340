...
/****************************************************************
   ...
   RETURNS:    returns size of passed value, 0 by default

   NOTES:      returns the size of node
****************************************************************/
...
/****************************************************************
   ...
   FUNCTION:   private version of size()
...
unsigned binTree<T>::size( Node<T>* p) const
{
	if (p != 0)
	{
		return 1 + size(p->left) + size(p->right); /* take size of both sides and add to root (1)*/
	}
